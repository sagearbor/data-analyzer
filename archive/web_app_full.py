        ...
        try:
            ...
            if file_format.lower() == "csv":
                ...
                try:
                    ...
                    self._debug_print(f"🔧 DEBUG: Loaded JSON data type: {type(json_data)}", debug)
                    ...
                    if isinstance(json_data, list):
                        ...
                        # Dictionary with potential nested structure
                        if len(json_data) == 1:
                    ...
                    self._debug_print(f"🔧 DEBUG: JSON DataFrame shape: {df.shape}", debug)
                    self._debug_print(f"🔧 DEBUG: JSON columns: {list(df.columns)}", debug)
                except json.JSONDecodeError as e:
                    raise ValueError(f"Invalid JSON format: {e}")
            elif file_format.lower() in ["excel", "xlsx", "xls"]:
                # Handle Excel format (base64 encoded)
                try:
                    import base64
                    excel_bytes = base64.b64decode(data_content)
                    df = pd.read_excel(io.BytesIO(excel_bytes))
                    self._debug_print(f"🔧 DEBUG: Excel DataFrame shape: {df.shape}", debug)
                    self._debug_print(f"🔧 DEBUG: Excel columns: {list(df.columns)}", debug)
                except Exception as e:
                    raise ValueError(f"Error reading Excel format: {e}")
            elif file_format.lower() == "parquet":
                # Handle Parquet format (base64 encoded)
                try:
                    import base64
                    parquet_bytes = base64.b64decode(data_content)
                    df = pd.read_parquet(io.BytesIO(parquet_bytes))
                    self._debug_print(f"🔧 DEBUG: Parquet DataFrame shape: {df.shape}", debug)
                    self._debug_print(f"🔧 DEBUG: Parquet columns: {list(df.columns)}", debug)
                except Exception as e:
                    raise ValueError(f"Error reading Parquet format: {e}")
            else:
