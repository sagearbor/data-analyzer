...
Supports multi-format analysis with AI-powered dictionary parsing
            ...
            with st.expander(f"Rule {i+1}", expanded=True):
                ...
                if rule_type == "range":
                    ...
                    allowed_text = st.text_input(
                        ...
                        value=",".join(map(str, entry["config"].get("allowed", []))),
                        key=f"allowed_{i}"
                    )
                    if allowed_text:
...
def render_about_page():
    ...
    - Currently supports CSV, JSON, Excel, and Parquet.
    ...
    with st.expander("🔄 View Detailed Data Flow"):
        st.markdown("""
        ```
        1. User uploads data file (CSV, etc.)
           ↓
    ...
    st.markdown("---")
    st.markdown("""
    **Version**: 1.1.0 | **License**: MIT | **GitHub**: [data-analyzer](https://github.com/sagearbor/data-analyzer)
    """)

def render_home_page():
    """Render the main Home page with data analysis functionality"""
    st.title("📊 Multi-Format Data Quality Analyzer")
    st.markdown("Upload a data file and configure validation rules to check data quality")
    ...
    with st.sidebar:
        ...
        demo_format = st.selectbox(
            ...
            ["CSV - Western", "CSV - Asian", "JSON - Mixed", "CSV - Clinical Trial", "Excel", "Parquet"],
...
S007,SITE04,2023-02-20,2023-03-20,55,F,INVALID_BMI,Placebo,None,131.7,90.0,Y,2.1"""
                ...
                df_excel = pd.DataFrame({
                    "id": [1, 2, 3, 4, 5], "name": ["John Doe", "Jane Smith", "Bob Johnson", "Alice Brown", "Charlie Wilson"],
                    "age": [25, 30, 35, 28, 150], "country": ["USA", "CAN", "MEX", "USA", "INVALID"],
                    "salary": [50000, 55000, 60000, 52000, 75000], "hire_date": ["2023-01-15", "2022-03-20", "2021-07-10", "invalid_date", "2020-12-05"]
                })
                output = io.BytesIO()
        ...
        dict_demo_format = st.selectbox(
            "Load demo dictionary:", ["None"] + list(DEMO_DICTIONARIES.keys()),
            key="dict_demo_selector", help="Load a demo dictionary that matches the demo datasets"
        )
        if dict_demo_format != "None":
            action = st.radio("Action:", ["Load Only", "Load & Parse", "Load, Parse & Apply"], key="demo_dict_action", horizontal=True, index=2)
        ...
        dict_file = st.file_uploader("Or upload your own:", type=['csv', 'xlsx', 'json', 'txt', 'pdf'], key="dict_uploader", help="Upload a data dictionary. Supports CSV, Excel, JSON, text, PDF, and REDCap formats.")
        if dict_file:
            upload_action = st.radio("Action on upload:", ["Load Only", "Load & Parse", "Load, Parse & Apply"], key="upload_dict_action", horizontal=True, index=2)
            file_extension = dict_file.name.split('.')[-1].lower()
            if file_extension == 'pdf':
                try:
                    import PyPDF2
                    pdf_reader = PyPDF2.PdfReader(dict_file)
                    dict_content = "".join(page.extract_text() for page in pdf_reader.pages)
        ...
        if st.session_state.get('dict_content'):
            should_parse = st.session_state.get('auto_parse', False) or st.button("🤖 Parse Dictionary", key="parse_dict_btn", use_container_width=True, type="primary")
            if should_parse:
                if 'auto_parse' in st.session_state: del st.session_state['auto_parse']
                with st.spinner("Parsing data dictionary with AI..."):
                    dict_content = st.session_state.get('dict_content', '')
                    format_hint = 'csv' if st.session_state.get('dict_source') == 'demo' else st.session_state.get('dict_format', 'auto')
                    format_hint = {'txt': 'text', 'xlsx': 'excel'}.get(format_hint, format_hint)
                    client = get_mcp_client(use_real_mcp=use_real_mcp)
                    result = asyncio.run(client.parse_data_dictionary(dictionary_content=dict_content, format_hint=format_hint, use_cache=True, debug=debug_mode))
                    if result.get('success'):
                        ...
                        st.session_state['parser_code'] = result.get('parser_code', '')
                        ...
                        if debug_mode and result.get('validation'):
                            with st.expander("Debug Info"): st.json(result['validation'])
            if st.session_state.get('parse_complete') and st.session_state.get('parsed_schema'):
                with st.expander("📋 Extracted Schema", expanded=True): st.json(st.session_state['parsed_schema'])
                if st.session_state.get('parsed_rules'):
                    with st.expander("⚙️ Extracted Rules", expanded=True): st.json(st.session_state['parsed_rules'])
                should_apply = st.session_state.get('auto_apply', False) or st.button("✅ Apply to Validation", key="apply_dict", type="primary", use_container_width=True)
                if should_apply:
                    if 'auto_apply' in st.session_state: del st.session_state['auto_apply']
                    if st.session_state.get('parsed_schema'):
                        st.session_state.schema_entries = [{'column': k, 'type': v} for k, v in st.session_state['parsed_schema'].items()]
                    if st.session_state.get('parsed_rules'):
                        rules_entries = []
                        for col_name, col_rules in st.session_state['parsed_rules'].items():
                            if 'allowed' in col_rules:
                                rules_entries.append({'column': col_name, 'rule_type': 'allowed_values', 'config': {'allowed': col_rules['allowed']}})
                            elif 'min' in col_rules or 'max' in col_rules:
                                rules_entries.append({'column': col_name, 'rule_type': 'range', 'config': {'min': col_rules.get('min'), 'max': col_rules.get('max')}})
            ...
            if st.session_state.get('parser_code'):
                ...
                base_name = st.session_state.get('dict_format', 'dictionary').replace(' ', '_').replace('-', '_') if st.session_state.get('dict_source') == 'demo' else st.session_state.get('dict_filename', 'dictionary').split('.')[0]
                st.download_button(label="📥 Download Parser Code (Python)", data=st.session_state['parser_code'], file_name=f"parser_{base_name}.py", mime="text/x-python", key="download_parser_code", use_container_width=True, help="Download the AI-generated Python code for this dictionary")

    # Main content area
    if uploaded_file is not None:
        try:
            file_format = uploaded_file.name.split('.')[-1].lower()
            if file_format in ['xlsx', 'xls']: file_format = 'excel'
            file_bytes = uploaded_file.read()
            file_content = file_bytes.decode(encoding) if file_format == 'csv' else base64.b64encode(file_bytes).decode()
            st.info(f"File: {uploaded_file.name} ({len(file_bytes):,} bytes, Format: {file_format.upper()})")
            with st.expander("📄 Data Preview", expanded=True):
                try:
                    if file_format == 'csv': preview_df = pd.read_csv(io.StringIO(file_content))
                    elif file_format == 'json': preview_df = pd.read_json(io.StringIO(file_content))
                    elif file_format == 'excel': preview_df = pd.read_excel(io.BytesIO(base64.b64decode(file_content)))
                    elif file_format == 'parquet': preview_df = pd.read_parquet(io.BytesIO(base64.b64decode(file_content)))
                    else: raise ValueError(f"Unsupported format for preview: {file_format}")
            ...
            with tab1: schema = create_schema_editor()
            with tab2: rules = create_rules_editor()
            with tab3:
                ...
                if st.button(f"🔍 Analyze {file_format.upper()} Data", type="primary", use_container_width=True):
                    with st.spinner(f"Analyzing {file_format.upper()} data..."):
                        client = get_mcp_client(use_real_mcp=use_real_mcp)
                        results = asyncio.run(client.analyze_data(data_content=file_content, file_format=file_format, schema=schema or None, rules=rules or None, min_rows=min_rows, debug=debug_mode))
        ...
        with st.expander("📄 Example Data (Editable)", expanded=True):
            try:
                if file_format in ['csv', 'json']:
                    edited_data = st.text_area("Edit your data here:", value=file_content, height=200, help=f"Edit the {file_format.upper()} data directly.")
                    if edited_data != file_content:
                        st.session_state.example_data = edited_data
                        file_content = edited_data
                else:
                    st.info(f"Binary format ({file_format.upper()}) - editing not supported. Preview only.")
        ...
        with tab3:
            st.subheader("🚀 Run Analysis")
            if st.button(f"🔍 Analyze {file_format.upper()} Example Data", type="primary", use_container_width=True):
                with st.spinner(f"Analyzing {file_format.upper()} data..."):
                    client = get_mcp_client(use_real_mcp=use_real_mcp)
                    results = asyncio.run(client.analyze_data(data_content=file_content, file_format=file_format, schema=schema or None, rules=rules or None, min_rows=min_rows, debug=debug_mode))
                    display_results(results)
...
def main():
    ...
    if option_menu:
        selected = option_menu(
            menu_title=None, options=["🏠 Home", "ℹ️ About"], icons=None, menu_icon=None, default_index=0, orientation="horizontal",
            styles={
                "container": {"padding": "5px", "background-color": "#f0f2f6", "border-radius": "10px", "margin-bottom": "1rem"},
                "icon": {"display": "none"},
                "nav-link": {"font-size": "16px", "text-align": "center", "margin": "0px 5px", "padding": "10px 20px", "border-radius": "5px", "--hover-color": "#e0e2e6"},
                "nav-link-selected": {"background-color": "#667eea", "color": "white", "font-weight": "bold"},
            }
        )
        selected = selected.replace("🏠 ", "").replace("ℹ️ ", "")
    else:
        selected = st.radio("Navigation", ["Home", "About"], horizontal=True, label_visibility="collapsed")
